	...
	SEMICOLON.widget = {
		...
		notifications: function( element ){
			...
			var notifyElement = $(element),
				...
				notifyExtendedTimeout = notifyElement.attr('data-notify-extended-timeout'),
				notifyCloseButton = notifyElement.attr('data-notify-close');

			if( !notifyPosition ) { notifyPosition = 'toast-top-right'; } else { notifyPosition = 'toast-' + notifyElement.attr('data-notify-position'); }
			if( !notifyMsg ) { notifyMsg = 'Please set a message!'; }
			if( !notifyTimeout ) { notifyTimeout = 0; }
			if (!notifyExtendedTimeout ) { notifyExtendedTimeout = 0; }
			if( notifyCloseButton == 'true' ) { notifyCloseButton = true; } else { notifyCloseButton = false; }

			toastr.options.positionClass = notifyPosition;
			toastr.options.timeOut = Number( notifyTimeout );
			toastr.options.extendedTimeOut = Number( notifyExtendedTimeout );
